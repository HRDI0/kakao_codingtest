...
# 예를 들어, 채팅방에 "Muzi"와 "Prodo"라는 닉네임을 사용하는 사람이 
# 순서대로 들어오면 채팅방에는 다음과 같이 메시지가 출력된다.
...
#     answer = []
#     for data in record:
#         action, uid = data.split()[0],data.split()[1]
#         if action != 'Leave':
#             action, uid, nick = data.split()
#             answer.append([action,uid,nick])
...
#                 if n[1] == uid:
#                     i_nick = n[2]
#             answer.append([action,uid,i_nick])
#             for i in range(len(answer)):
#                 if uid == answer[i][1]:
#                     answer[i][2] = nick
#     result = []
...
#     for st in answer:
#         if st[0] == 'Enter':
#             result.append('{0}님이 들어왔습니다.'.format(st[2]))
#         elif st[0] == 'Leave':
#             result.append('{0}님이 나갔습니다.'.format(st[2]))
#     return result
#############################################################3
def solution(record):
    answer = []
    ...
    for data in record:
        ...
        user_action.append([action,uid])
        if action != 'Leave':
            nick = data[2]
            user_id[uid] = nick
    for log in user_action:
        if log[0] == 'Enter':
            answer.append(f'{user_id[log[1]]}님이 들어왔습니다.')
        elif log[0] == 'Leave':
            answer.append(f'{user_id[log[1]]}님이 나갔습니다.')
    return answer
